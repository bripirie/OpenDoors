/* EX_VOTE3.C - The files ex_vote1.c thru ex_vote5.c demonstrate the         */
...
void AddQuestion(void)
{
       ...
       QuestionRecord.nTotalAnswers++)
   {
      ...
      od_printf("%2d: ", QuestionRecord.nTotalAnswers + 1);
...
int ChooseQuestion(int nFromWhichQuestions, char *pszTitle, int *nLocation)
{
   ...
   for(;;)
   {
         ...
         ++nCurrent)
      {
         ...
         od_printf("%c. %s\n\r", chCurrent,
            szQuestionName[nCurrent + nPagedToQuestion]);
      }
...
void DisplayQuestionResult(tQuestionRecord *pQuestionRecord)
{
   ...
   od_printf("Question created by %s on %s\n\r", pQuestionRecord->szCreatorName,
